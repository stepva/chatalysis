        ...
        if "reactions" in m:
            for r in m["reactions"]:
                ...
                if name in names:
                    ...
                    reactions["got"][name][reaction] = 1 + reactions["got"][name].get(
                        reaction, 0
                    )
                    if r["actor"] in names:
                        reactions["gave"][r["actor"]]["total"] += 1
                        reactions["gave"][r["actor"]][reaction] = 1 + reactions["gave"][
                            r["actor"]
                        ].get(reaction, 0)
...
def topTen(path: str) -> "tuple[dict[str, int], dict[str, int]]":
    ...
    groups = {}
    inboxes = [
        f"{path}/{m}/inbox/" for m in os.listdir(path) if m.startswith("messages")
    ]
            ...
            if file.startswith("message") and file.endswith(".json"):
                with open(n + "/" + file) as data:
                    ...
                    if data["thread_type"] == "Regular":
                        ...
                        groups[conversationName] = len(data["messages"]) + groups.get(
                            conversationName, 0
                        )
