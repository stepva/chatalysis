...
def groupchat_names_plot(group_names: list[dict[str, Any]], from_day: date, to_day: date) -> str:
    ...
    df["from_str"] = df["from"].dt.strftime("%Y-%m-%d")
    df["to_str"] = df["to"].dt.strftime("%Y-%m-%d")  # for some reason the d3 formatting did not work for this column
    df["type"] = "Chat name"
    ...
    fig1 = px.timeline(
        ...
        text="group_name",
        custom_data=["from_str", "to_str", "group_name", "changed_by"],
        height=100,
    ...
    fig1.update_traces(
        ...
        hovertemplate="<b>%{customdata[2]}</b><br>%{customdata[0]} - %{customdata[1]}<br>Changed by %{customdata[3]}<extra></extra>",
    )
...
def nicknames_plot(nicknames: list[dict[str, Any]], from_day: date, to_day: date) -> str:
    ...
    fig1 = px.timeline(
        ...
        text="nickname",
        custom_data=["from_str", "to_str", "nickname", "changed_by"],
        range_x=[max([from_day, datetime(2015, 12, 18).date()]), to_day],
        height=max(75, min([50 * n_targets, 250])) if n_targets < 10 else n_targets * 25,
    )
    fig1.update_layout(
        showlegend=False,
        plot_bgcolor="rgba(0, 0, 0, 0)",
